void __SoftWareUart_Init()
{
  ...
  SoftWare_Uart_Tx=1;//ensure idel state for standard
  delay_us(BITPERIOD);
}
...
{
  char mask;
  SoftWare_Uart_Tx = 0;//replace by one for inverted
  delay_us(BITPERIOD);

  for (mask=0x01;mask!=0;mask=mask<<1)
  {
    if(Data&mask) SoftWare_Uart_Tx = 1;//replace by zero for inverted
    else SoftWare_Uart_Tx = 0;//replace by one for inverted
    delay_us(BITPERIOD);
  }
  SoftWare_Uart_Tx = 1;//replace by zero for inverted
  delay_us(BITPERIOD);
}
...
unsigned char __Rx_Byte()
{
  ...
  char Data;
  Data=0;
  while(SoftWare_Uart_Rx);//replace by (!SoftWare_Uart_Rx) for inverted
  delay_us(HALFBITPERIOD);
  for(mask=0x01;mask!=0;mask=mask<<1)
  {
    delay_us(BITPERIOD);
    if(SoftWare_Uart_Rx)Data=Data|mask;//replace by (!SoftWare_Uart_Rx) for inverted
  }
  Delay_us(BITPERIOD);
  return Data;
}
